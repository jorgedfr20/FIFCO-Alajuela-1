...
# ─────────────────────────────────────────────
def generate_trips_heuristic(deliveries, distance_matrix, capacity, speed_kmh, time_per_stop, time_per_pallet, max_trip_time=480):
    """
    Nearest-neighbor + bin-packing heuristic.
    Groups deliveries into trips respecting capacity AND 8-hour (480 min) time limit.
    ...
    for i, d in enumerate(sorted_deliveries):
        ...
        while remaining_cap > 0:
            ...
            for j, d2 in enumerate(sorted_deliveries):
                ...
                if d2["total"] > remaining_cap:
                    continue
                # Check if adding this stop would exceed the 8-hour trip limit
                candidate_nodes = trip_nodes + [d2["node"]]
                candidate_route = [0] + candidate_nodes + [0]
                candidate_dist  = calculate_route_distance(candidate_route, distance_matrix)
                candidate_time  = calculate_trip_time(
                    candidate_dist, len(candidate_nodes),
                    trip_total + d2["total"], speed_kmh, time_per_stop, time_per_pallet
                )
                if candidate_time > max_trip_time:
                    continue  # Skip: would exceed 8-hour jornada
        ...
        elif time_min > max_trip_time:
            estado = "⚠️ Supera 8 h"  # Solo trips dedicados que por distancia no pueden reducirse
...
# ─────────────────────────────────────────────
def try_generate_trips_ortools(deliveries, distance_matrix, capacity, speed_kmh, time_per_stop, time_per_pallet, max_trip_time=480):
    """
...
def generate_trips(deliveries, distance_matrix, capacity, speed_kmh, time_per_stop, time_per_pallet, use_ortools, max_trip_time=480):
    """Main dispatcher. max_trip_time enforces the 8-hour (480 min) per-trip limit."""
...
def main():
    ...
    trips, method = generate_trips(
        ...
        params["use_ortools"], max_trip_time=params["max_jornada"]
    )
